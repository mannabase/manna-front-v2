...
import { BehaviorSubject, from, Observable, of, catchError, tap, map } from 'rxjs';
...
export class MetamaskBrightIdService {
  ...
  checkBrightIdState(): Observable<BrightIdState> {
    return from(
      userVerificationStatus('Manna', `${this.account$.value}`, {
        signed: 'eth',
  ...
  signMessage(message: string): Observable<string>  {
    return from((window as any).ethereum.request({ method: 'personal_sign', params: [message, this.account$.value] }) as Promise<string>);
  }
}
